...
google-apps-script/Code.gs 구글 시트 자동 연동용 코드 (script.google.com에 복사해서 사용)
...
회원가입 신청과 참가 신청이 올 때마다 구글 시트에도 자동으로 한 줄씩
기록되게 할 수 있습니다. Netlify Forms는 계속 원본으로 남고, 구글 시트는
"보기 편한 사본"입니다 — 명단을 정렬·필터링하거나 회비 납부 여부 같은
컬럼을 직접 추가해서 관리하고 싶을 때 유용합니다.

**이 작업은 사용자님의 구글 계정으로 직접 하셔야 합니다** (제가 대신 로그인해서
설정할 수 없는 부분이라, 아래 순서 그대로만 따라 하시면 됩니다).

1. [sheets.google.com](https://sheets.google.com) 에서 새 스프레드시트 생성
   (이름은 자유롭게, 예: "EMPK 회원·참가 신청 명단")
2. 상단 메뉴 **확장 프로그램 → Apps Script** 클릭
3. 열린 편집기에 원래 있던 코드를 전부 지우고, 이 저장소의
   [`google-apps-script/Code.gs`](google-apps-script/Code.gs) 파일 내용을 그대로 복사해서 붙여넣기
4. 저장(디스크 아이콘 또는 `Cmd+S`)
5. 오른쪽 위 **배포 → 새 배포** 클릭
6. 톱니바퀴 아이콘 → **유형 선택 → 웹 앱**
7. 설정:
   - **실행 방식(Execute as)**: 나 (본인 계정)
   - **액세스 권한(Who has access)**: **모든 사용자(Anyone)**
     (이렇게 해야 홈페이지 방문자의 브라우저가 로그인 없이 이 주소로 보낼 수 있습니다)
8. **배포** 클릭 → 처음이면 구글이 권한 승인을 요청합니다 →
   본인 계정으로 승인 (`고급` → `안전하지 않은 페이지로 이동`이 떠도
   본인이 만든 스크립트이므로 진행해도 안전합니다)
9. 배포가 끝나면 나오는 **웹 앱 URL**을 복사
   (`https://script.google.com/macros/s/........./exec` 형태)
10. 홈페이지 `/admin` → **학회 소개 / 기본 정보** → **연동 (구글 시트 자동 기록)**
    → **구글 시트 웹앱 주소**에 방금 복사한 URL 붙여넣기 → 저장

이제 회원가입·참가 신청이 올 때마다 처음 만든 스프레드시트에 "회원가입",
"참가신청" 탭이 자동으로 생기고 한 줄씩 쌓입니다.

**알아두실 점**
- 광고 차단 확장 프로그램이 `script.google.com` 요청을 막으면 그 신청 건은
  시트에 안 쌓일 수 있습니다. 이런 경우에도 Netlify Forms에는 항상 기록되니,
  신청자 수를 확인할 땐 Netlify Forms를 기준으로 삼아주세요.
- 스크립트를 나중에 고치고 싶으면 script.google.com에서 코드를 수정한 뒤
  **배포 → 배포 관리 → 수정(연필 아이콘) → 새 버전** 순서로 다시 배포해야
  변경사항이 반영됩니다 (그냥 저장만 하면 기존 배포에는 반영되지 않습니다).
- 웹 앱 URL은 비밀번호가 아니라 "누구나 데이터를 보낼 수 있는 주소"입니다.
  다만 이 주소를 아는 사람은 원칙적으로 가짜 신청 데이터를 시트에 넣을 수도
  있으니, URL을 SNS 등에 공개적으로 올리지는 마세요.
